  ...
  return (
    <div className="space-y-6">
      <div className="relative overflow-hidden cyber-card p-8 md:p-12 min-h-[300px] md:min-h-[400px] group hover:shadow-[0_0_60px_oklch(0.72_0.20_195_/_0.2)] transition-all duration-700">
        ...
        <svg className="absolute bottom-0 left-0 w-full h-full opacity-10 pointer-events-none" style={{ zIndex: 1 }}>
          <line x1="0" y1="80%" x2="40%" y2="80%" className="circuit-line" />
          <line x1="40%" y1="80%" x2="40%" y2="20%" className="circuit-line" />
          <circle cx="40%" cy="20%" r="4" fill="var(--primary)" className="animate-pulse" />
        </svg>
        ...
        <div className="relative z-10">
          <div className="mb-6">
            ...
            <p className="text-sm md:text-base text-muted-foreground uppercase tracking-wider max-w-2xl">
              Join a community of elite traders leveraging AI-powered strategies to dominate the market
            </p>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-8">
            <div className="bg-background/60 backdrop-blur-sm border border-primary/30 p-4 jagged-corner-small hover:scale-105 hover:border-primary/60 transition-all group/stat relative overflow-hidden">
              ...
              <div className="text-2xl font-black text-primary relative z-10">#{profile.rank}</div>
              <div className="absolute bottom-0 left-0 right-0 h-1 bg-primary/20">
                <div className="h-full bg-primary" style={{ width: `${100 - (profile.rank / 100) * 100}%` }} />
              </div>
            </div>
            <div className="bg-background/60 backdrop-blur-sm border border-primary/30 p-4 jagged-corner-small hover:scale-105 hover:border-primary/60 transition-all group/stat relative overflow-hidden">
              <div className="absolute inset-0 bg-gradient-to-br from-primary/5 to-transparent opacity-0 group-hover/stat:opacity-100 transition-opacity" />
              ...
              <div className="text-2xl font-black text-primary relative z-10">{profile.level}</div>
              <div className="absolute bottom-0 left-0 right-0 h-1 bg-primary/20">
                <div className="h-full bg-accent" style={{ width: `${(profile.level / 50) * 100}%` }} />
              </div>
            </div>
            <div className="bg-background/60 backdrop-blur-sm border border-primary/30 p-4 jagged-corner-small hover:scale-105 hover:border-primary/60 transition-all group/stat relative overflow-hidden">
              <div className="absolute inset-0 bg-gradient-to-br from-primary/5 to-transparent opacity-0 group-hover/stat:opacity-100 transition-opacity" />
              ...
              <div className="text-2xl font-black text-primary relative z-10">{profile.wins}</div>
              <div className="absolute bottom-0 left-0 right-0 h-1 bg-primary/20">
                <div className="h-full bg-secondary" style={{ width: `${(profile.wins / 200) * 100}%` }} />
              </div>
            </div>
            <div className="bg-background/60 backdrop-blur-sm border border-accent/40 p-4 jagged-corner-small hover:scale-105 hover:border-accent/70 hover:shadow-[0_0_20px_oklch(0.68_0.18_330_/_0.4)] transition-all group/stat relative overflow-hidden">
              ...
              <div className="text-2xl font-black text-accent neon-glow-secondary relative z-10">{profile.streak}</div>
              <div className="absolute bottom-0 left-0 right-0 h-1 bg-accent/20">
                <div className="h-full bg-accent" style={{ width: `${(profile.streak / 10) * 100}%` }} />
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="glass-morph-card p-6 md:p-8 relative overflow-hidden group hover:shadow-[0_0_60px_oklch(0.68_0.18_330_/_0.15)] transition-all duration-500">
        ...
        <svg className="absolute inset-0 w-full h-full opacity-10 pointer-events-none" style={{ zIndex: 1 }}>
          ...
          <circle cx="20%" cy="30%" r="3" fill="var(--accent)" className="animate-pulse" />
          <circle cx="80%" cy="70%" r="3" fill="var(--primary)" className="animate-pulse" style={{ animationDelay: '0.5s' }} />
        </svg>
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-6 relative z-10">
          ...
          <div className="cyber-card-accent px-6 py-3 relative group/balance hover:scale-105 transition-transform">
            ...
            <div className="text-xl md:text-2xl font-black text-accent flex items-center gap-2 neon-glow-secondary">
              <Star size={20} weight="fill" className="text-accent animate-pulse" />
              {profile.xp} XP
            </div>
            <div className="absolute bottom-0 left-0 right-0 h-1 bg-accent/20">
              ...
              {perk.unlocked && (
                <div className="absolute top-3 right-3 z-10">
                  <div className="bg-background/90 border-2 border-accent rounded-full p-2 animate-pulse-glow">
                    <ShieldCheck size={24} weight="fill" className="text-accent neon-glow-secondary" />
                  </div>
                </div>
              )}
              
              {!perk.unlocked && (
                <div className="absolute top-0 right-0 px-3 py-1 bg-destructive/90 border-b-2 border-l-2 border-destructive" style={{ borderRadius: '0 8px 0 8px' }}>
                  <span className="text-xs font-bold text-destructive-foreground uppercase tracking-wider">Limited Offer</span>
                </div>
              )}
              ...
              <div className="relative p-6 flex flex-col h-full">
                <div className="mb-4">
                  ...
                  <h3 className="text-xl md:text-2xl font-black uppercase tracking-tight text-background mb-2 flex items-center gap-2">
                    {perk.title}
                    {!perk.unlocked && (
                      <Lightning size={18} weight="fill" className="text-background/70 animate-pulse" />
                    )}
                  </h3>
                  ...
                  <div className="space-y-1">
                    {perk.details.split(',').map((detail, i) => (
                      <div key={i} className="flex items-center gap-2 text-xs text-background/70 font-medium">
                        <CheckCircle size={14} weight="fill" className="text-background/70 flex-shrink-0" />
                        {detail.trim()}
                      </div>
                    ))}
                  </div>
                ...
                <div className="mt-auto pt-4 space-y-3">
                  {!perk.unlocked && (
                    <div className="flex items-center justify-between px-3 py-2 bg-background/20 border border-background/30 relative overflow-hidden">
                      <span className="text-xs font-bold text-background/80 uppercase tracking-wider">Cost</span>
          ...
          <Card className="backdrop-blur-md bg-card/50 border-primary/30 h-full shadow-[0_0_30px_oklch(0.72_0.20_195_/_0.1)] hover:shadow-[0_0_40px_oklch(0.72_0.20_195_/_0.2)] transition-all duration-500 relative overflow-hidden">
            ...
            <CardHeader className="relative z-10">
              <div className="flex items-center gap-3 mb-2">
                ...
                <CardTitle className="text-2xl font-black uppercase tracking-tight">Season Leaderboard</CardTitle>
              </div>
                ...
                {leaderboard.map((entry) => (
                  ...
                  >
                    ...
                    <div className="flex items-center gap-4 relative z-10">
                      <div className={`text-3xl font-black w-12 h-12 flex items-center justify-center relative ${
                        entry.rank === 1 ? 'text-accent neon-glow-secondary scale-110' :
                        entry.rank === 2 ? 'text-secondary scale-105' :
                        entry.rank === 3 ? 'text-primary scale-105' :
                      ...
                      <div className="flex-1">
                        <div className="font-bold text-sm md:text-base flex items-center gap-2">
                          {entry.user}
                          {entry.user === profile.username && (
                            <Badge className="bg-accent/20 border-accent/40 text-accent text-[9px] px-1.5 py-0.5">YOU</Badge>
                          )}
                        </div>
                        <div className="flex items-center gap-3 text-xs text-muted-foreground">
                          <span className="flex items-center gap-1">
                            <Trophy size={12} weight="fill" className={entry.rank <= 3 ? 'text-accent' : ''} />
                            Lvl {entry.level}
                          </span>
                          <span>•</span>
                          <span className="flex items-center gap-1">
                            <Target size={12} weight="fill" className={entry.rank <= 3 ? 'text-primary' : ''} />
                            {entry.wins}W
                          </span>
                        </div>
                      </div>
                      <div className="text-right">
                        <div className={`font-black text-lg ${
                          entry.rank === 1 ? 'text-accent neon-glow-secondary text-xl' : 
                          entry.rank === 2 ? 'text-secondary text-lg' : 
            ...
            <CardHeader className="relative z-10">
              <div className="flex items-center gap-3 mb-2">
                <div className="p-2 bg-primary/20 border border-primary/40 jagged-corner-small">
                  <Rocket size={28} weight="duotone" className="text-primary animate-pulse-glow" />
                </div>
            ...
            <CardContent className="space-y-4 relative z-10">
              <div>
                ...
                <div className="flex items-center justify-between mt-2">
                  ...
                  <span className="text-xs font-bold text-accent">{xpProgress.toFixed(1)}%</span>
                </div>
              </div>
              ...
              <div className="pt-4 border-t border-border/50">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div className="p-2 bg-primary/10 border border-primary/30 text-center">
                    <div className="text-lg font-black text-primary">{profile.wins}</div>
                    <div className="text-[9px] text-muted-foreground uppercase tracking-wider">Total Wins</div>
                  </div>
                  <div className="p-2 bg-accent/10 border border-accent/30 text-center">
                    <div className="text-lg font-black text-accent">{profile.streak}</div>
                    <div className="text-[9px] text-muted-foreground uppercase tracking-wider">Win Streak</div>
                  </div>
                </div>
              </div>
            </CardContent>
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  <h3 className="font-black text-sm uppercase tracking-wide mb-1 flex items-center gap-2">
                    Keep Trading!
                    <Fire size={16} weight="fill" className="text-accent animate-pulse" />
                  </h3>
                  <p className="text-xs text-muted-foreground mb-3">
                    Complete <span className="text-accent font-bold">3 more</span> successful trades to earn <span className="text-accent font-bold">100 XP</span> bonus
                  </p>
                  <div className="flex gap-2 mt-3">
                    ...
                    <div className="w-full h-1.5 bg-muted/30 relative overflow-hidden">
                      <div className="h-full bg-accent/50 animate-pulse" style={{ width: '0%', animationDelay: '0.2s' }} />
                    </div>
                    <div className="w-full h-1.5 bg-muted/30 relative overflow-hidden">
                      <div className="h-full bg-accent/50 animate-pulse" style={{ width: '0%', animationDelay: '0.4s' }} />
                    </div>
                  </div>
                  <div className="flex items-center justify-between mt-2">
                    <span className="text-[10px] text-muted-foreground uppercase tracking-wider">1/3 Complete</span>
                    <span className="text-[10px] font-bold text-accent uppercase tracking-wider">+100 XP</span>
                  </div>
                </div>
              </div>
            </CardContent>
