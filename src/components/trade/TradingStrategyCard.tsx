...
export default function TradingStrategyCard({
  strategy,
  userTier,
  onToggle,
  onUpgradeClick,
}: TradingStrategyCardProps) {
        ...
        <div className="p-5 relative">
          <div className="flex items-start justify-between mb-3">
            <div className="flex-1">
                ...
                <Tooltip>
                  ...
                  <TooltipContent className="cyber-card-accent border-2 border-accent max-w-sm p-4 z-50 angled-corner-tr">
                    <div className="space-y-2">
                      <div className="font-bold uppercase tracking-wider text-accent text-sm">
                        {strategy.name}
                      </div>
              ...
              <div className="flex items-center gap-2 mb-2">
                <Badge
                  className={`${riskColors[strategy.risk]} bg-transparent text-[9px] uppercase tracking-wider px-2 py-0.5 cut-corner-tr`}
                >
                  {strategy.risk} Risk
                </Badge>
                <Badge
                  className={`${tierColors[strategy.requiredTier as keyof typeof tierColors]} bg-transparent text-[9px] uppercase tracking-wider px-2 py-0.5 cut-corner-tr`}
                >
          ...
          {!isLocked && strategy.pnl !== undefined && (
            <div className="grid grid-cols-4 gap-2 pt-3 border-t border-border/50">
              <div>
                ...
                <div
                  className={`text-sm font-bold ${strategy.pnl! >= 0 ? "text-primary" : "text-destructive"}`}
                >
                  {strategy.pnl! >= 0 ? "+" : ""}$
              ...
              <div>
                ...
                <div className="flex items-center gap-1">
                  ...
                  <span
                    className={`text-sm font-bold ${strategy.pnlPercent! >= 0 ? "text-primary" : "text-destructive"}`}
                  >
