...
export default function Analytics() {
  ...
  const performance: PerformanceSummary = {
    ...
    bestTrade: { amount: 420.50, symbol: 'BTC' },
    worstTrade: { amount: -85.20, symbol: 'ETH' },
    avgTrade: 12.50,
    sharpeRatio: 1.85,
    totalTrades: 234,
    winRate: 68.5
  ...
  const strategies: StrategyPerformance[] = [
    { name: 'Momentum', trades: 45, winRate: 68, avgReturn: 2.3, totalPnL: 850.40 },
    { name: 'Mean Reversion', trades: 32, winRate: 71, avgReturn: 1.8, totalPnL: 620.30 },
    { name: 'DCA', trades: 120, winRate: 65, avgReturn: 0.9, totalPnL: 980.50 }
  ]
  ...
  const recentTrades = [
    { date: '11/15', symbol: 'BTC', side: 'buy' as const, amount: 0.01, price: 42000, pnl: 124, pnlPercent: 2.95 },
    { date: '11/15', symbol: 'ETH', side: 'sell' as const, amount: 0.5, price: 2200, pnl: -42, pnlPercent: -1.9 },
    { date: '11/14', symbol: 'SOL', side: 'buy' as const, amount: 10, price: 48.50, pnl: 85, pnlPercent: 17.5 }
  ]
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-1 text-sm">
                ...
                <span className="text-primary">+{performance.totalPnLPercent}%</span>
              </div>
            </div>
          ...
          <div className="mt-4 flex items-center justify-between text-sm">
            <div className="flex items-center gap-4">
              <div className="flex items-center gap-2">
                ...
                <span className="data-label">BEFORE_TAX: ${grossPnL.toFixed(2)}</span>
              </div>
              <div className="flex items-center gap-2">
              ...
              {strategies.map((strategy) => (
                <div key={strategy.name} className="p-4 bg-muted/30 border-l-2 border-primary hover:bg-muted/50 transition-all">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-primary font-bold">${strategy.totalPnL.toFixed(2)}</span>
                  </div>
                  <div className="grid grid-cols-3 gap-4 text-xs">
                    ...
                    <div>
                      <div className="data-label">WIN_RATE</div>
                      <div className="text-primary font-bold">{strategy.winRate}%</div>
                    </div>
                    <div>
                      <div className="data-label">AVG_RETURN</div>
                      <div className="text-accent font-bold">+{strategy.avgReturn}%</div>
                    </div>
                  </div>
                </div>
              ))}
            ...
            <div className="grid grid-cols-2 gap-3">
              {portfolioComposition.map((item) => (
                <div key={item.asset} className="flex items-center gap-2">
                  ...
                  <span className="data-label">{item.asset}: {item.percent}%</span>
                </div>
              ))}
            </div>
          </div>
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-3 gap-4">
                <div className="space-y-2">
                  ...
                  <div className="text-xl font-bold text-primary">${grossPnL.toFixed(2)}</div>
                </div>
                <div className="space-y-2">
                ...
                <div className="space-y-2">
                  ...
                  <div className="text-xl font-bold text-accent">${netPnL.toFixed(2)}</div>
                </div>
              </div>
            ...
            {recentTrades.map((trade, i) => (
              <div key={i} className="flex items-center justify-between p-4 bg-muted/30 border-l-2 border-primary hover:bg-muted/50 transition-all">
                ...
                <div className="text-right">
                  <div className={`text-lg font-bold ${trade.pnl && trade.pnl > 0 ? 'text-primary' : 'text-destructive'}`}>
                    {trade.pnl && trade.pnl > 0 ? '+' : ''}${trade.pnl?.toFixed(2)}
                  </div>
                  <div className={`text-xs ${trade.pnlPercent && trade.pnlPercent > 0 ? 'text-primary' : 'text-destructive'}`}>
                    {trade.pnlPercent && trade.pnlPercent > 0 ? '+' : ''}{trade.pnlPercent}%
                  </div>
                </div>
              </div>
            ))}
