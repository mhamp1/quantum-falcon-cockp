...
const activityTypes = [
  { action: 'Analyzed SOL/USDC pair', type: 'analysis' as const, result: 'success' as const },
  { action: 'Executed DCA buy order', type: 'trade' as const, result: 'success' as const, profit: 12.45 },
  { action: 'Detected volatility spike', type: 'alert' as const, result: 'warning' as const },
  { action: 'Optimized strategy parameters', type: 'optimization' as const, result: 'success' as const },
  { action: 'Scanned new token launches', type: 'analysis' as const, result: 'success' as const },
  { action: 'Placed limit order', type: 'trade' as const, result: 'pending' as const },
  { action: 'Risk threshold adjusted', type: 'optimization' as const, result: 'success' as const },
  { action: 'Market pattern identified', type: 'analysis' as const, result: 'success' as const },
]
...
export default function Agents() {
  ...
  const getActivityIcon = (type: ActivityLog['type']) => {
    switch (type) {
      ...
      case 'analysis': return ChartLine
      case 'alert': return Warning
      case 'optimization': return ArrowsClockwise
  ...
  const getResultIcon = (result: ActivityLog['result']) => {
    switch (result) {
      ...
      case 'pending': return ArrowsClockwise
    }
  }
  ...
  const getActivityColor = (type: ActivityLog['type']) => {
    switch (type) {
      ...
      case 'analysis': return 'text-primary'
      case 'alert': return 'text-destructive'
      case 'optimization': return 'text-accent'
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                  ...
                  <div className="flex items-start justify-between mb-4">
                    <div className="flex items-start gap-4 flex-1">
                      <div className={`p-3 jagged-corner-small border-2 transition-all relative ${
                        ...
                        <Icon size={32} weight="duotone" className={agent.enabled ? 'text-secondary' : 'text-muted-foreground'} />
                        {agent.enabled && (
                          <div className="absolute -top-1 -right-1">
                            <div className="relative">
                              <Lightning size={16} weight="fill" className="text-secondary animate-pulse-glow" />
                            </div>
                          </div>
                        )}
                      </div>
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-muted-foreground uppercase tracking-wide mb-4">{agent.description}</p>
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div className="flex-1">
                            <div className="flex justify-between mb-1">
                              ...
                              <span className="text-xs font-bold text-primary">{agent.xp} / {agent.xpToNext}</span>
                            </div>
                            <Progress value={(agent.xp / agent.xpToNext) * 100} className="h-1.5 bg-muted" />
                          </div>
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    ...
                    <div className="p-3 jagged-corner bg-muted/30 border border-primary/30 relative overflow-hidden group hover:border-primary/50 transition-all">
                      ...
                      <div className="relative z-10">
                        ...
                        <p className="text-2xl font-bold text-secondary neon-glow-secondary hud-value">+${agent.metrics.profitContribution.toFixed(2)}</p>
                      </div>
                    </div>
                  </div>
        ...
        <div className="space-y-4">
          <div className="cyber-card-accent scan-line-effect">
            <div className="p-4 relative z-10">
              <div className="flex items-center gap-2 mb-4">
                <Lightning size={20} weight="fill" className="text-accent animate-pulse-glow" />
                <h3 className="text-lg font-bold uppercase tracking-[0.15em] text-accent hud-text">LIVE ACTIVITY</h3>
              </div>
              <div className="space-y-2 max-h-[600px] overflow-y-auto pr-2 scrollbar-thin">
                {activityLog.length === 0 ? (
                  <div className="text-center py-8">
                    <ArrowsClockwise size={32} className="text-muted-foreground mx-auto mb-2 animate-spin" />
                    <p className="text-xs text-muted-foreground uppercase tracking-wider">Waiting for agent activity...</p>
                  </div>
                ) : (
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-start gap-2">
                          <ActivityIcon size={16} weight="duotone" className={`${getActivityColor(activity.type)} mt-0.5 flex-shrink-0`} />
                          ...
                          <ResultIcon 
                            ...
                            className={`flex-shrink-0 mt-0.5 ${
                              activity.result === 'success' ? 'text-secondary' : 
                              activity.result === 'warning' ? 'text-destructive' : 
          ...
          <div className="cyber-card scan-line-effect">
            <div className="p-4 relative z-10">
              <h3 className="text-lg font-bold uppercase tracking-[0.15em] text-primary hud-text mb-4">PERFORMANCE</h3>
              
              <div className="space-y-4">
                <div className="p-3 jagged-corner bg-muted/30 border border-primary/30 text-center">
                  <p className="text-[10px] text-muted-foreground mb-1 uppercase tracking-[0.15em] font-semibold">Total Trades</p>
                ...
                <div className="p-3 jagged-corner bg-muted/30 border border-secondary/30 text-center">
                  <p className="text-[10px] text-muted-foreground mb-1 uppercase tracking-[0.15em] font-semibold">Combined Profit</p>
                ...
                <div className="p-3 jagged-corner bg-muted/30 border border-primary/30 text-center">
                  <p className="text-[10px] text-muted-foreground mb-1 uppercase tracking-[0.15em] font-semibold">Avg Confidence</p>
                  <p className="text-3xl font-bold text-primary neon-glow hud-value">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="cyber-card group hover:scale-105 transition-all cursor-pointer">
          <div className="p-4 relative z-10 text-center">
            ...
            <p className="text-xs text-muted-foreground uppercase tracking-[0.15em] mb-1 font-semibold">Success Rate</p>
            <p className="text-4xl font-bold text-primary neon-glow hud-value">94.2%</p>
          </div>
        </div>
        ...
        <div className="cyber-card-accent group hover:scale-105 transition-all cursor-pointer">
          <div className="p-4 relative z-10 text-center">
            ...
            <p className="text-xs text-muted-foreground uppercase tracking-[0.15em] mb-1 font-semibold">Precision Score</p>
            <p className="text-4xl font-bold text-accent neon-glow-accent hud-value">8.7/10</p>
          </div>
        </div>
        ...
        <div className="cyber-card group hover:scale-105 transition-all cursor-pointer">
          <div className="p-4 relative z-10 text-center">
            ...
            <p className="text-xs text-muted-foreground uppercase tracking-[0.15em] mb-1 font-semibold">AI Models</p>
            <p className="text-4xl font-bold text-secondary neon-glow-secondary hud-value">12</p>
          </div>
        </div>
      </div>
    </div>
